...
exports.run = async (client, message, args) => {
  if (!message.member.permissions.any(["ADMINISTRATOR", "MANAGE_CHANNELS"])) {
    return message.channel.send("You don't have any rights to do this.");
  }
  ...
  let toSecond = Math.floor(convert / 10000); // This will convert the ms to s. (seconds)
  ...
  return message.channel.send(`<#${channel.id}> has been slowing down for **${ms(ms(time), {long: true})}**.`);
}

exports.help = {
  name: "slowmode",
  description: "Slowing down the channel.",
  usage: "r.slowmode [channel] <time>",
  example: "r.slowmode #general 5s \nr.slowmode 5.25 hrs"
}
