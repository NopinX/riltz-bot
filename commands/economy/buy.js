...
const Discord = require("discord.js")
    ...
    exports.run = async(client, message, args) => {
        ...
        if (!args[0]) {
            message.channel.send('what do you want to buy tho. to see list do d!shop')
        }
      ...
      if(args[0] === "crowbar") {
        if(author < 500) {
          return message.channel.send(`You need at least 500$ Coins to buy crowbar`)
        } else {
          let items = db.fetch(message.author.id, { items: [] })
          db.push(message.author.id, "crowbar")
          message.channel.send("You bought 1x Crowbar")
